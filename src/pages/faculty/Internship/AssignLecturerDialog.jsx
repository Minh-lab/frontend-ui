...
export default function AssignLecturerDialog({ isOpen, onClose, selectedCount, onSuccess, selectedIds = [] }) {
  ...
  const loadLecturers = useCallback(async () => {
    try {
      setIsLoading(true);
      const response = await internshipService.getLecturers(searchQuery);
      if (response.success) {
  ...
  const handleAssign = async (lecturerId) => {
    ...
    try {
      setIsAssigning(true);
      const response = await internshipService.assignLecturer(selectedIds, lecturerId);
      if (response.success) {
      ...
      toast.error("Lỗi phân công giảng viên");
